...
export function JourneyView({
  ...
  const rows = Array.from({ length: 4 }, (_, index) =>
    roadmap.weeks.slice(index * 6, index * 6 + 6),
  );
    ...
    <div className="view-stack journey-view">
      <section className="journey-hero">
        <div className="hero-copy">
          ...
          <p>
            22 semanas para transformar a ementa do Itaú em domínio prático,
            portfólio público e segurança na sabatina.
          </p>
          <div className="hero-actions">
            <button className="primary-button" onClick={() => onSelectWeek(currentWeek)}>
              Continuar missão <ArrowRight size={17} />
            </button>
            <button className="secondary-button" onClick={onStartFocus}>
              <TimerReset size={17} /> Foco de 25 min
            </button>
          </div>
        </div>
      ...
      <section className="mission-strip" aria-label="Resumo da missão">
        <div className="mission-stat">
          <span className="stat-icon cyan"><Target size={18} /></span>
          <div><strong>{greenItems}<small>/72</small></strong><span>itens verdes</span></div>
        </div>
        <div className="mission-stat">
          <span className="stat-icon violet"><Clock3 size={18} /></span>
          <div><strong>{hours}h {minutes.toString().padStart(2, "0")}m</strong><span>tempo focado</span></div>
        </div>
        <div className="mission-stat">
          <span className="stat-icon coral"><Flame size={18} /></span>
          <div><strong>{workspace.sessions.length ? 1 : 0} dia</strong><span>sequência atual</span></div>
        </div>
        <div className="mission-stat">
          <span className="stat-icon green"><FolderGit2 size={18} /></span>
          <div><strong>{Object.values(workspace.projectStatus).filter((s) => s === "publicado").length}<small>/22</small></strong><span>projetos publicados</span></div>
        </div>
        <div className="xp-stat">
          <span>NÍVEL {Math.floor(workspace.xp / 500) + 1}</span>
          <strong>{workspace.xp} XP</strong>
          <div className="mini-progress"><i style={{ width: `${workspace.xp % 500 / 5}%` }} /></div>
        </div>
      </section>

      <section className="section-heading">
        <div>
          <span className="eyebrow muted">MAPA DA EXPEDIÇÃO</span>
          <h2>22 missões. Uma evolução contínua.</h2>
        </div>
      ...
      <section className="roadmap-layout">
        <div className="quest-board" aria-label="Roadmap de 22 semanas">
          <div className="board-skyline" aria-hidden="true">
            <span>Fundamentos</span><span>Modelagem</span><span>Inteligência</span><span>Consolidação</span>
          </div>
            ...
            {rows.map((row, rowIndex) => (
              <div className={`road-row ${rowIndex % 2 ? "reverse" : ""}`} key={rowIndex}>
                {row.map((week) => {
                  ...
                  return (
                    <button
                      className={`week-node status-${status}`}
                      style={{ "--node-color": color } as React.CSSProperties}
                      key={week.number}
                      onClick={() => onSelectWeek(week)}
                      aria-label={`Semana ${week.number}: ${week.title}`}
                    >
                      <span className="node-beacon">
                        {week.number === currentWeekNumber && <span className="avatar-dot"><Sparkles size={13} /></span>}
                        <b>{week.number}</b>
                      </span>
          ...
          <div className="board-legend">
            ...
            <span><i className="legend-open" /> Disponível</span>
            <span><i className="legend-green" /> Verde</span>
            <span><i className="legend-locked" /> Próxima</span>
          </div>
        </div>
        ...
        <aside className="current-mission-card">
          ...
          <div className="mission-goal">
            <div><span>Meta semanal</span><strong>{Math.floor(studyMinutes / 60)}h de {workspace.settings.weeklyGoalHours}h</strong></div>
            <div className="wide-progress"><i style={{ width: `${goalPercent}%` }} /></div>
          </div>
          <div className="mission-checklist">
            <span><BookOpen size={15} /> {currentWeek.content.length} tópicos</span>
            <span><MessageCircleQuestion size={15} /> 10 perguntas</span>
            <span><FolderGit2 size={15} /> {currentWeek.project.title}</span>
          </div>
          <button className="primary-button full" onClick={() => onSelectWeek(currentWeek)}>
            Abrir central da semana <ArrowRight size={16} />
          </button>
        </aside>
      </section>

      <section className="section-heading compact">
        <div><span className="eyebrow muted">ATALHOS</span><h2>O que você quer avançar agora?</h2></div>
      </section>
      <section className="quick-action-grid">
        {quickActions.map((action) => {
          const Icon = action.icon;
          return (
            <button key={action.id} className="quick-card" onClick={() => action.id === "pomodoro" ? onStartFocus() : onNavigate(action.id)}>
              <span className={`quick-icon ${action.accent}`}><Icon size={21} /></span>
