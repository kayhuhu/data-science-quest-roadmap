...
export function QuestApp({
  ...
  const commandResults = useMemo(() => {
    ...
    const questions = roadmap.weeks.flatMap((week) => week.sabatina.map((item, index) => ({ ...item, week: week.number, index, id: `q-${week.number}-${index}` })))
    ...
    <div className={`quest-app ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className={`main-sidebar ${mobileMenu ? "mobile-open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          ...
          {!collapsed && <div className="source-seal"><Sparkles size={16} /><div><strong>roadmap-v19</strong><span>22 semanas · 72 itens</span></div></div>}
        </div>
