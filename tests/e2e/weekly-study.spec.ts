...
test("renders exactly four tabs with readable typography and no overflow", async ({ page }) => {
  const tabs = page.locator(".weekly-four-tabs button");
  await expect(tabs).toHaveCount(4);
  for (const label of ["ESTUDAR", "PRATICAR", "SABATINA", "REVISAR"]) await expect(tabs.filter({ hasText: label })).toHaveCount(1);
...
test("switches every weekly section inside the same study page", async ({ page }) => {
  ...
  for (const [tab, heading] of checks) {
    await page.locator(".weekly-four-tabs button", { hasText: tab }).click();
...
test("persists compact mastery evidence after reload", async ({ page }) => {
  await page.locator(".weekly-four-tabs button", { hasText: "REVISAR" }).click();
